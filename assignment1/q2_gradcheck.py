...
def your_sanity_checks():
    ...
    ### YOUR CODE HERE
    quad = lambda x : sigmoid_function(x)
    print "Running sanity checks..."
    gradcheck_naive(quad, np.array([[1.0, 2.0], [-1.0, -2.0]]))
    gradcheck_naive(quad, np.random.randn(3, ))  # 1-D test
    gradcheck_naive(quad, np.random.randn(4, 5))  # 2-D test
    ### END YOUR CODE

def sigmoid_function(x):
    temp = q2_sigmoid.sigmoid(x)
    grad = q2_sigmoid.sigmoid_grad(temp)
    return (temp, grad)
